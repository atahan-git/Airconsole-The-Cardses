...
public class PowerUpDealer : NetworkBehaviour {
	...
	void RpcPoisonExplosionEffect () {
		...
		var temp = (GameObject)Instantiate (PowerUpStuff.s.TrapCardExplosionEffect, ScoreBoardManager.s.scoreBoards[myPlayer.id].transform);
		temp.transform.position = ScoreBoardManager.s.scoreBoards [myPlayer.id].transform.position;
	}
	...
	void ActiveEffect (GameObject playerEffect, GameObject othersEffect) {
		if (!isLocalPlayer) {
			othersActiveEffect = (GameObject)Instantiate(othersEffect, ScoreBoardManager.s.scoreBoards[myPlayer.id].transform);
